...
# The live six-model swarm (E8) is re-evaluated from data/live_cache; to regenerate
# the live answers themselves run experiments/live_swarm.py first (several hours).
set -e
cd "$(dirname "$0")/.."
...
for s in main zoo llm history swarm ext; do
  stamp "study $s"
